...
- **H0:** μ ≥ 45 (population mean meets or exceeds 45 mpg)
- **H1:** μ < 45 (population mean is below 45 mpg)
- **Test:** One-tailed, lower tail critical; α = .01 (Type I error is serious for the manufacturer)
- **Sample:** n = 6 cars (small for computational simplicity)

### Why t Instead of z?

- The **population standard deviation (σ)** is unknown.
- σ must be **estimated** with the sample standard deviation (s).
- Estimating σ changes the sampling distribution and requires the **t test** instead of the z test.
...
- **sX̄** = estimated standard error of the mean = s/√n

### Gas Mileage Example

- X̄ = 43, μhyp = 45, sX̄ = 0.89
- t = (43 − 45) / 0.89 = **−2.25**
- Critical t = −3.365 (df = 5, one-tailed, α = .01)
- **Decision:** Retain H0 because −2.25 > −3.365 (less negative than critical)
...
- This makes it slightly harder to reject H0 when using t—correctly accounting for the uncertainty in estimating σ.
...
- If the observed result qualifies as a **rare outcome** under H0 → **reject H0**.
- Otherwise → **retain H0**.
...
## 13.6 Details: Estimating the Standard Error (sX̄)
...
**Important:** Use **n − 1** (not n) when dividing SS to obtain s; use **n** when taking the square root for sX̄.
...
where:
- **tconf** = critical value from the t table for the desired level of confidence and df = n − 1
- Use the **two-tailed** panel of Table B (confidence intervals are symmetrical like two-tailed tests)

### Gas Mileage Example

- X̄ = 43, sX̄ = 0.89, df = 5
- 95% confidence: tconf = 2.571 (from Table B, two-tailed panel)
...
| **Estimated standard error** | sX̄ = s/√n |

| **Degrees of freedom** | df = n − 1 |
| **t vs. z** | t has fatter tails; critical t > critical z |
| **t ratio** | t = (X̄ − μhyp) / sX̄ |
| **CI for μ** | X̄ ± (tconf)(sX̄) when σ unknown |
...
| **t ratio** | (X̄ − μhyp) / sX̄; replaces z when σ is estimated |
| **Degrees of freedom (df)** | Number of values free to vary given one or more restrictions; df = n − 1 for one sample |
| **Estimated standard error (sX̄)** | s/√n; used when σ is unknown |
